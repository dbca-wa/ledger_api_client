...
class TokenPaymentDetailCheckout(TemplateView):
    """
        Token authenticated alternative to PaymentDetailCheckout

        Link and token generated by authorised payment administrator and sent to user

        Token then verified and decoded via another ledger endpoint, with which the basket id is retrieved    
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        if token:
            ...
            if not basket_id:
                context['data'] = render_to_string('payments/gateway-error.html', {'error': 'Could not check out basket. Access token may be invalid or expired.'}) 
                return render(request, self.template_name, context)

            basket_totals = ledger_api_client_utils.get_basket_total(basket_id)
            if 'data' in basket_totals:
                if 'basket_total' in basket_totals['data']:
                    payment_total = Decimal(basket_totals['data']['basket_total'])
